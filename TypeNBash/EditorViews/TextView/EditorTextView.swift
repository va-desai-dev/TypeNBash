...
final class EditorTextView: NSTextView, CurrentLineHighlighting, MultiCursorEditing {
    ...
    override func viewDidMoveToWindow() {
        ...
        // (TypeNBash: window background tint via DocumentWindow omitted — not a CotEditor window here.)
    ...
    override func setSelectedRanges(_ ranges: [NSValue], affinity: NSSelectionAffinity, stillSelecting stillSelectingFlag: Bool) {
        ...
        if !stillSelectingFlag, !self.isShowingCompletion {
            ...
            if self.highlightsBraces {
                self.highlightMatchingSymbol(candidates: SymbolPair.braces)
            }
    ...
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
        if self.window?.firstResponder === self, self.performEditorShortcut(event) { return true }
        return super.performKeyEquivalent(with: event)
    }

    override func keyDown(with event: NSEvent) {
        if self.performEditorShortcut(event) { return }
        super.keyDown(with: event)
    }

    /// Keep source-editing shortcuts local to this responder, away from terminals
    /// and the fields in the shared find panel.
    private func performEditorShortcut(_ event: NSEvent) -> Bool {
        guard !self.hasMarkedText() else { return false }
        let modifiers = event.modifierFlags.intersection([.command, .control, .option, .shift])
        let key = event.charactersIgnoringModifiers ?? ""
        if modifiers == .command {
            switch key {
                case "/" where self.isEditable && !self.commentDelimiters.isEmpty: self.toggleComment(nil)
                case "]" where self.isEditable: self.shiftRight(nil)
                case "[" where self.isEditable: self.shiftLeft(nil)
                case "f":
                    let item = NSMenuItem()
                    item.tag = TextFinder.Action.showFindInterface.rawValue
                    self.performEditorTextFinderAction(item)
                case "g": self.matchNext(nil)
                default: return false
            }
            return true
        }
        if modifiers == [.command, .shift], key.lowercased() == "g" {
            self.matchPrevious(nil)
            return true
        }
        if modifiers == .control, key == " ", self.isEditable {
            self.complete(nil)
            return true
        }
        if modifiers == [.command, .option], self.isEditable {
            switch event.keyCode {
                case 126: self.moveLineUp(nil)
                case 125: self.moveLineDown(nil)
                default: return false
            }
            return true
        }
    ...
    override func menu(for event: NSEvent) -> NSMenu? {
        ...
        // `super.menu(for:)` lets AppKit's NSTextCheckingController annotate the menu for the
        // clicked word, which calls `NSSpellChecker.hasLearnedWord(_:)`. That blocks the main
        // thread on a synchronous XPC round-trip to the spell server, and since the reply is
        // delivered at default QoS the Thread Performance Checker flags it as a priority
        // inversion. Spell checking is disabled in this editor, so those annotations are dead
        // weight: build the menu from the class default instead, and cache it because the
        // result no longer depends on the event.
        if let cachedContextMenu = self.cachedContextMenu { return cachedContextMenu }

        // -> `Self.defaultMenu` is shared with every other text view, and `self.menu` returns
        //    that same instance, so copy it before customizing. `NSMenu.copy()` copies
        //    submenus too, which the Substitutions customization below relies on.
        guard let menu = Self.defaultMenu?.copy() as? NSMenu else { return nil }
        ...
        menu.items.removeAll { item in
            item.submenu?.items.contains {
                ...
                $0.action == #selector(makeBaseWritingDirectionNatural)  // Writing Direction submenu
            } ?? false
        }
        menu.items.removeAll { $0.action == #selector(toggleQuickLookPreviewPanel) }
    ...
    @IBAction func selectEnclosingSymbols(_ sender: Any?) {
        ...
        guard
            ...
            let enclosingRange = self.string.rangeOfEnclosingSymbolPair(at: selectedRange, candidates: SymbolPair.braces + [.ltgt])
        else { return }
    ...
    private func invalidateNonContiguousLayout() {
        ...
        // disable non-contiguous layout on vertical layout (2016-06, OS X 10.11 - macOS 26)
        // -> Otherwise by vertical layout, the view scrolls occasionally a bit on typing.
        self.layoutManager?.allowsNonContiguousLayout = (self.layoutOrientation == .vertical)
            ? false
...
extension EditorTextView {
    ...
    override var rangeForUserCompletion: NSRange {
        ...
        guard !self.string.isEmpty, range.location != NSNotFound else { return range }
        ...
        guard !invalidRange.isNotFound else { return NSRange(location: 0, length: range.upperBound) }
