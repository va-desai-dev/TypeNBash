...
struct FindPanelResultView: View {
    ...
    /// Dismisses the result list.
    ///
    /// TypeNBash shows this view in a popover from the inline find bar, so the
    /// owner passes its own dismissal in. CotEditor sent an action to the find
    /// panel's split view controller, which no longer exists here.
    var onClose: () -> Void
    ...
    var body: some View {
        ...
        VStack(alignment: .leading) {
            HStack {
                Button(.init("Close", table: "TextFind", comment: "button label"), systemImage: "chevron.up", action: self.onClose)
                .fontWeight(.medium)
                .imageScale(.small)
                .labelStyle(.iconOnly)
                .help(.init("Close find result.", table: "TextFind", comment: "tooltip"))
                
                Text("Found \(self.model.matches.count) matches in “\(self.documentName ?? "Unknown").”",
...
private extension NSTextView {
    ...
    var documentName: String? {
        ...
        // TypeNBash: no DocumentWindowController; the editor is not a CotEditor document window.
        nil
    }
}
...
#Preview {
    ...
    return FindPanelResultView(model: model, onClose: { })
}
